...
namespace Devarc
{
    public class JsonTableBuilder : BaseTableBuilder
    {
        Regex mRegex = new Regex(@"^[0-9]*(?:\.[0-9]*)?$");

        public void Build(SettingConfig cfg, string inputPath, EXPORT_TYPE exportType)
        {
            ...
            for (int i = 0; i < doc.NumberOfSheets; i++)
            {
                ...
                readHeader(sheet);
                onReadSheet(cfg, sheet, exportType);
            }
        }

        void onReadSheet(SettingConfig cfg, ISheet sheet, EXPORT_TYPE exportType)
        {
            ...
            if ((exportType & EXPORT_TYPE.Client) != 0)
            {
                ...
                var folderName = isResource ? cfg.client_resource : cfg.client_bundle;
