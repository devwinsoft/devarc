...
namespace Devarc
{
    public class CsTableBuilder : BaseTableBuilder
    {
        public void Build(SettingConfig cfg, string inputPath, EXPORT_TYPE exportType)
        {
            var doc = open(inputPath);
            var fileName = Path.GetFileNameWithoutExtension(inputPath);
            var fileNameEx = $"{fileName}.cs";

            using (var sw = File.CreateText(fileNameEx))
            {
            ...
            Console.WriteLine($"Generate file: {fileNameEx}");
